...
* **时间**：2017-07-07
* **描述**:
> 1. 并行M1钱包的无线与刷卡2中绑定方式
> 2. 开始绑定增加对PIN_KEY自动的按照随机数生成，并增加停止绑定指令
> 3. 剔除对导入白名单时对UID的复制，不允许改变硬件的UID

* **时间**：2017-07-17
* **描述**:
> 1. 改变无线绑定的KEY的产生原则，为接收器的的后四位数字
> 2. 修复刷卡绑定指令返回答题器ID为全0的BUG

* **时间**：2017-07-19
* **描述**:
> 1. 修复刷卡绑定指令返回答题器ID为全0的BUG
